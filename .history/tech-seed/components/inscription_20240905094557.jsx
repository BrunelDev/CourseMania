...
export function Inscription({ button }) {
  const [username, setUsername] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [password2, setPassword2] = useState("");
  const [showMessage, setShowMessage] = useState(false);
  const [status, setStatus] = useState("eleve");
  const [message, setMessage] = useState("");
  const Message = () => {
    return (
      <div className="fixed bottom-0 left-0 w-full bg-white p-4 rounded-md shadow-md">
  ...
  const registration = () => {
    ...
    register(email, password, username, status)
      .then((res) => {
        setMessage(res);
        setShowMessage(true);
      })
